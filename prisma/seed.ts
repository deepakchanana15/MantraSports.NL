/**
 * Mantra Sports NL — Database Seed
 * Run: npm run db:seed
 *
 * Creates:
...
async function main() {
  console.log('🌱 Seeding Mantra Sports NL database…\n')

  // ── 1. Super Admin User ─────────────────────────────────────
  const adminEmail = 'admin@mantrasports.nl'
  ...
  for (const category of DEFAULT_CATEGORIES) {
    await prisma.category.upsert({
      ...
      create: {
        ...
        metaTitle: `${category.name} | Mantra Sports NL`,
        metaDescription: `Shop premium ${category.name.toLowerCase()} from Mantra Sports Netherlands. Professional quality, competitive prices.`,
      },
      update: {
        name: category.name,
        sortOrder: category.sortOrder,
  ...
  console.log('   Email:       admin@mantrasports.nl')
